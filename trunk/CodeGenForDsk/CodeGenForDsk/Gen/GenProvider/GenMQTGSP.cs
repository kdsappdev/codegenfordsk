...
namespace CodeGenForDsk.CodeGenForDsk.Gen.GenProvider
{
    ...
    //MQ TG_SP
    //</summary>
    internal class GenMQTGSP : AbstractGen
    {
        public override string GenCode(DataRow drTable, DataRow[] drTableColumns, Gen.GenOptionInfo goi)
        {
            ...
            string prodcedureName = goi.ProdcedureName.Replace("$TABLENAME", tableName);
            string triggerName = goi.TriggerName.Replace("$TABLENAME", tableName);
            string topic = goi.TopicName.Replace("$TABLENAME", tableName);
        ...
        private static void gen1(DataRow[] drTableColumns, StringBuilder sb, string tableName, string prodcedureName, string triggerName,string topic, bool isExistLoation)
        {
            #region MQ存储过程
            ...
            foreach (DataRow dr in drTableColumns)
            {
                ...
                sb.AppendFormat("\t").AppendFormat("   || '").AppendFormat(Utils.CodeGenHelper.StrProperty(columnName).ToUpper()).AppendFormat("\"'").AppendFormat("\r\n");
        ...
        private static void gen2(DataRow[] drTableColumns, StringBuilder sb, string tableName, string prodcedureName, string triggerName,string topic, bool isExistLoation)
        {
            #region MQ存储过程
            sb.AppendFormat("CREATE OR REPLACE PROCEDURE").AppendFormat(" ").AppendFormat(prodcedureName, tableName).AppendFormat(" (").AppendFormat("\r\n");
            sb.AppendFormat("\t").AppendFormat("p_table    IN   VARCHAR2,").AppendFormat("\r\n");
